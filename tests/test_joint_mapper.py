"""Tests for JointMapper (policy/joint_mapper.py)."""
import numpy as np
import pytest
...
class TestDefaults:
    def test_default_all_joints(self):
        ...
        assert m.n_policy == 29
        assert m.n_robot == 29
        np.testing.assert_array_equal(m.policy_indices, np.arange(29))

    def test_policy_joints_subset(self):
        """policy_joints as a subset of robot joints."""
        legs = G1_29DOF_JOINTS[:12]
        m = JointMapper(G1_29DOF_JOINTS, policy_joints=legs)
        assert m.n_policy == 12
        assert m.n_robot == 29
        assert m.policy_joints == legs
...
class TestMapping:
    ...
    def test_isaaclab_reordering(self):
        ...
        m = JointMapper(
            G1_29DOF_JOINTS,
            policy_joints=ISAACLAB_CONFIG_ORDER,
        )
        np.testing.assert_array_equal(
            m.policy_indices, np.array(ISAACLAB_TO_NATIVE_INDICES)
        )

    def test_robot_to_policy_values(self):
        ...
        obs = m.robot_to_policy(robot_vals)
...
class TestPartialControl:
    ...
    def test_partial_control_7_arm(self):
        ...
        m = JointMapper(G1_29DOF_JOINTS, policy_joints=right_arm)
        assert m.n_policy == 7
...
class TestValidation:
    def test_invalid_joint_raises(self):
        ...
        with pytest.raises(ValueError, match="Unknown"):
            JointMapper(G1_29DOF_JOINTS, policy_joints=["fake_joint"])
    ...
    def test_duplicate_joint_raises(self):
        ...
        with pytest.raises(ValueError, match="Duplicate"):
            JointMapper(
                G1_29DOF_JOINTS,
                policy_joints=["left_hip_pitch", "left_hip_pitch"],
            )

    def test_empty_controlled_raises(self):
        """Empty controlled list raises ValueError."""
        with pytest.raises(ValueError, match="empty"):
            JointMapper(G1_29DOF_JOINTS, policy_joints=[])
