"""Tests for MirrorRobot (read-only DDS subscriber for mirror mode).
...
from unitree_launcher.robot.mirror_robot import (
    MirrorRobot,
    _MOTOR_MODE_SERVO,
    _NUM_MOTOR_IDL_HG,
    _WATCHDOG_TIMEOUT_S,
...
def robot_29dof(config_29dof):
    """MirrorRobot instance (29-DOF, not connected)."""
    return MirrorRobot(config_29dof)


@pytest.fixture
def robot_23dof(config_23dof):
    """MirrorRobot instance (23-DOF, not connected)."""
    return MirrorRobot(config_23dof)


@pytest.fixture
def robot_with_cmd(robot_29dof):
    """MirrorRobot with mock publisher and cmd message injected (no DDS connect)."""
    robot = robot_29dof
...
class TestMirrorRobotWatchdog:
    ...
    def test_real_robot_connect_timeout(self, config_29dof):
        ...
        with patch("unitree_launcher.robot.mirror_robot.patch_unitree_threading"), \
            ...
            import unitree_launcher.robot.mirror_robot as rr_mod
...
class TestMirrorRobotPublishThread:
...
class TestMirrorRobotGracefulShutdown:
